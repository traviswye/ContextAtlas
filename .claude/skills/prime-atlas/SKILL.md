...
| Tool returns ERR not_found for sentinel symbol | ⚠ MCP connected but atlas substrate stale | The server imports `atlas.json` only into an empty local cache, when it starts (it warns at startup when `atlas.json` differs from what the cache holds). If `atlas.json` changed after the cache was filled (an `/index-atlas` run, a pull), another `/index-atlas` refresh cannot help: the user closes Claude Code (or disconnects `contextatlas` in `/mcp`), deletes the local cache file (`atlas.local_cache`, default `.contextatlas/index.db`) while no `contextatlas index` is running, and restarts or reconnects; it is rebuilt from `atlas.json` with no API calls. With `atlas.committed: true` nothing is lost; with `false` this discards anything only that cache holds. Otherwise suggest `/index-atlas` refresh; substrate may not reflect current code/ADR state |
...
| Atlas.json malformed JSON | ✗ Substrate corrupt | Restore `atlas.json` first: `git checkout -- <atlas.path>` (or resolve the merge conflict in it), then refresh with `/index-atlas` or `contextatlas index`. `contextatlas index`, even with `--full`, does not rebuild an invalid `atlas.json`: it stops at it and names the same remedy. With `atlas.committed: false`, `contextatlas index` never rewrites `atlas.json` (the local cache is the source of truth); if the file cannot be restored, delete it |
