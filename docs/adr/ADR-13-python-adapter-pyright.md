...
| `textDocument/publishDiagnostics` | Published automatically on `didOpen`. Standard LSP shape (severity / message / range / code / codeDescription). | Notification handler caches by URI. Per-call diagnostic-readiness uses the `waitForDiagnostics` helper (1000ms ceiling). Pyright emits no `$/progress` for analysis-completion in 1.1.409 + tested fixture sizes, so cold-start absorbs into the per-call ceiling rather than a separate init-time race — the pyright-fallback path of [ADR-18](ADR-18-lsp-adapter-readiness-pattern.md). | T6 |
