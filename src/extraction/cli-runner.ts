      ...
      if (validateResult.exitCode !== 0) {
        writeStderr(
          [
            ...
            "",
            "Re-run extraction after addressing the failing invariants. A plain",
            "`contextatlas index` re-extracts an ADR only when its file changed, so",
            "either edit the failing ADRs and run `contextatlas index`, or re-extract",
            "every ADR and docs page with:",
            "  contextatlas index --full",
            "Note: `--full` also re-extracts every docstring file (commits stay",
            "key-gated), which can be hundreds of model calls; the cost preview on",
            "stderr shows the planned calls before any is made. For an ADR-only",
            "rebuild, set `extraction.streams: [adr]` in .contextatlas.yml for that",
            "run and remove the key again afterwards (the other streams' claims are",
            "kept meanwhile).",
            "",
