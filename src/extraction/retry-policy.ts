/**
 * Retry policy for the extraction wrapper (`anthropic-client.ts`):
 * Anthropic SDK error recognition, retry/fail classification, and
 * backoff computation.
 *
 * Retry ownership (v1.2): the wrapper is the ONLY retry layer. It
 * sends every request with the SDK per-request option
 * `{ maxRetries: 0 }` (honored by SDK 0.128.0 and by the 0.32.x
 * clients the benchmarks repo passes in), so the SDK never retries
 ...
 *      (e.g. the benchmarks repo's own `new Anthropic()` passed into
 *      `createExtractionClient`): numeric `status`, or a connection-
